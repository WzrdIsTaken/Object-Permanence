...
* 
    template <typename FunctionType = details::default_function_type,
    typename ThreadType = std::jthread,
    typename QueueType = thread_safe_queue<FunctionType>>
    requires std::invocable<FunctionType>&&
    std::is_same_v<void, std::invoke_result_t<FunctionType>>
    class thread_pool {
    public:
        using priority_type = nabi::thread_safe_priority_queue<FunctionType>::priority_type;
        static priority_type constexpr null_priority = nabi::thread_safe_priority_queue<FunctionType>::null_priority;
        ...
        typename ReturnType = std::invoke_result_t<Function&&, Args &&...>>
        requires std::invocable<Function, Args...>
        [[nodiscard]] std::future<ReturnType> priority_enqueue(priority_type priority, Function f, Args... args) {
        current_enqueued_task_priority_ = priority;
        return enqueue(std::forward<Function>(f), std::forward<Args>(args)...);
    }
        ...
        requires std::invocable<Function, Args...>&&
        std::is_same_v<void, std::invoke_result_t<Function&&, Args &&...>>
        void priority_enqueue_detach(priority_type priority, Function&& func, Args &&...args) {
            current_enqueued_task_priority_ = priority;
            enqueue_detach(std::forward<Function>(func), std::forward<Args>(args)...);
        }
        ...
        private:
            ...
            void enqueue_task(Function&& f) {
                ...
                if constexpr (std::is_same_v < QueueType, nabi::thread_safe_priority_queue<FunctionType>) {
                    tasks_[i].tasks.push(std::forward<Function>(f), current_enqueued_task_priority_);
                    current_enqueued_task_priority_ = null_priority;
                }
                else {
                    tasks_[i].tasks.push(std::forward<Function>(f));
                }
            ...
            std::size_t count_{};
            std::atomic_int_fast64_t pending_tasks_{};
            std::atomic<priority_type> current_enqueued_task_priority_{ null_priority };
    };
*/
